...
ggsave(p.motifs.pos, filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/UMIposition.tiff", width = 10, height = 13, dpi = 300)
```
...
ggsave(p.cover, filename = paste0("/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/UMIinR2.Coverage.tiff"), bg = "white", dpi = 200, height = 6, width = 12)
```
...
ggpubr::compare_means(data = df.star %>% filter(GROUP == "FS-UMI\nCTAAC\nRetinal Organoids"), perc~TYPE , group.by = "reads", p.adjust.method = "bonferroni")
...
p.star <- ggplot() +
  ...
  geom_segment(data = data_frame(x = rep(1,3), y = 100), aes(x = 1, xend = x+1, y = 100, yend = 100)) +
  geom_text(data = data.frame(text = c("***", "***", "***"), x = rep(1.5,3), y = rep(100.5,3)), aes(x = x, y = y, label = text), size =6) + ggtitle("FS-UMI CTAAC")
...
p.star <- plot_grid(p.star, p.star.ss3, rel_widths = c(1,0.5))
ggsave(p.star, filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/UMIinR2.star.stats.tiff", dpi = 300, bg = "white", width = 12, height = 5)
```
...
ggpubr::compare_means(data = df.readDistrib %>% filter(GROUP == "FS-UMI\nCTAAC\nRetinal Organoids"), perc~TYPE , group.by = "distribution", p.adjust.method = "bonferroni")
...
p.readDistrib <- ggplot() +
    ...
    geom_segment(data = data_frame(
      x = rep(1,5), 
      y = rep(80, 5), 
      distribution = c("3'UTR\nExons","5'UTR\nExons","CDS\nExons","Intergenic","Introns")), aes(x = 1, xend = x+1, y = y, yend = y)) +
    geom_text(data = data_frame(
      x = rep(1.5,5), 
      y = c(82, rep(80.5, 4)), 
      text = c("ns", "***", "***", "***", "*"),
      distribution = c("3'UTR\nExons","5'UTR\nExons","CDS\nExons","Intergenic","Introns")), aes(x = x, y = y, label = text), size =6)
...
p.readDistrib.all <- plot_grid(p.readDistrib, p.readDistrib.SS3, rel_widths = c(1,0.5))
ggsave(p.readDistrib.all, filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/UMIinR2.resqc.stats.tiff", dpi = 300, bg = "white", width = 16, height = 5)
...
med <- pivot_longer(med, cols = c(count_0, count_1, count_5), names_to = "threshold", values_to = "counts") %>%
  mutate(GROUP = ifelse(grepl("316", ID), "FS-UMI\nCTAAC\nRetinal Organoids", "SS3\nMice Fibroblast\nHagemann-J. et al\nE-MTAB-8735"),
         threshold = str_replace(threshold, "_", ">"))

# 4. Visualize results
ggpubr::compare_means(data = med %>% filter(GROUP == "FS-UMI\nCTAAC\nRetinal Organoids"), counts~TYPE , group.by = "threshold")
...
p.counts.umi <- ggplot() + 
  ...
  geom_segment(data = data.frame(x = c(0.775, 1.8, 2.9), 
                                 y = rep(5000, 3), 
                                 text = c("**", "ns", "***"),
                                 TYPE = rep("R1", 3)), aes(x = x, xend = x+0.45, y = y, yend = y)) +
  geom_text(data = data.frame(x = c(0.775, 1.8, 2.9), 
                                 y = rep(5000, 3), 
                                 text = c("**", "ns", "***"),
                                 TYPE = rep("R1", 3)), aes(x = x+0.225, y = y+150, label = text), size = 6) + 
...
p.counts.umi.SS3 <- ggplot() + 
  ...
  xlab("") +
  scale_fill_brewer(palette = "Set1") +
  scale_y_continuous(breaks = seq(0,3000,500), limits = c(0,3500)) +
  ylab("Detected Genes") + ggtitle("SS3") +
  geom_text(aes(x = 1, y = 0, label = "n = 365"), size = 4) 


ggsave(p.counts.umi, filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/UMIinR2.counts.tiff", dpi = 300, bg = "white", width = 8, height = 5)
...
p.corrs <- ggplot(mycorrs, aes(x = "", y = tau, fill = "col")) + 
  ...
  theme(legend.position = "none") +
  xlab("") +
  ylab("R1/R2 UMI correlations (tau)")

ggsave(p.corrs, filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/UMIinR2.corrs.tiff", dpi = 300, bg = "white", width = 2, height = 5)
...
ggsave(p.motifs.pos, filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/UMIposition.singleEnd.tiff", width = 12, height = 14, dpi = 300)
```
...
# 1.6. Visualization
percentageUMI <- plot_grid(nrow = 1, rel_widths = c(0.35,0.65),
...
ggsave(percentageUMI, filename="/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/ORGANOIDS_percentageUMI.tiff", bg = "white", dpi = 300, width = 14, height = 5)
...
ggsave(p.dups, filename="/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/ORGANOIDS_percentageDups.tiff", bg = "white", dpi = 300, width = 6, height = 5)
```
