...
p.Length <- ggplot() + 
  ...
  xlab("") +
  scale_y_continuous(breaks = seq(2500,4500,500), label = comma) +
  theme_cowplot(font_size = 24) +
  theme(legend.position = "none") + 
  ggpubr::stat_pvalue_manual(p.signif, label = "p = {p.adj.signif}",  tip.length = 0.01, size = 4) 


ggsave(plot_grid(align = "v", p.GC + theme(legend.position = "none"), p.Length  + theme(legend.position = "none"), ncol = 2), filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/HEK_DE_explore.tiff", dpi = 450, height = 6, width = 12, bg = "white")
