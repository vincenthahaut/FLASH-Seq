...
p1.500K <- ggplot() +
  ...
  ggpubr::stat_pvalue_manual(p.signif, label = "p = {p.adj.signif}",  tip.length = 0.01, size = 6) +
  geom_text(data = data.frame(NAME = c("SS2", "SS3", "FS")), aes(y = c(200,200,200), x = c(1,2,3), label = c("n=80", "n=51", "n=105")), size = 6) 
...
# 2.4. Same as before but for FS - 5 uL
mydata.tmp <- mydata %>%
  filter(DEPTH == 500000 & NAME != "FS - 25 µL") %>%
  dplyr::select(count_5, count_0, NAME) %>%
  pivot_longer(-NAME, names_to = "threshold", values_to = "genes") %>%
  mutate(group = paste0(threshold, "_", NAME), 
         group = factor(group, levels = c(
           paste0("count_0_", c("SS2", "SS3", "FS - 5 µL (Miniaturized)")),
           paste0("count_5_", c("SS2", "SS3", "FS - 5 µL (Miniaturized)")))))
...
p1.500K <- ggplot() +
  ...
  geom_violin(data = mydata.tmp, aes(y = genes, x = group, fill = NAME)) +
  geom_boxplot(data = mydata.tmp, aes(y = genes, x = group, fill = NAME), width = 0.1, position = position_dodge(width = 0.9)) +
  scale_fill_manual(values = c("FS - 5 µL (Miniaturized)" = "#E41A1C",
                               "SS2" = "#377EB8",
                               "SS3" = "#4DAF4A")) +
  scale_y_continuous(breaks = seq(0,12000,1000)) +
  scale_x_discrete(labels = c(">0 read", ">5 reads")) +
...
# 2.3. Save results
ggsave(p1.500K, 
       filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/HEK_exonCounts_5uL.pdf", bg = "white", dpi = 450, width = 7, height = 6)
...
p.gene.content <- ggplot() +
  ...
  ylab("Genes Detected") +
  ggpubr::stat_pvalue_manual(p.signif, label = "p = {p.adj.signif}", step.group.by = "gene_type", step.increase = 0.025, tip.length = 0.01, size = 6) +
...
p.isoform.HEK <- ggplot() +
  ...
  ggpubr::stat_pvalue_manual(filter(p.signif, name == "TPM > 1"), label = "{p.adj.signif}",  step.increase = 0.03, tip.length = 0.01, size = 6) +
  ggpubr::stat_pvalue_manual(filter(p.signif, name == "TPM > 5"),, label = "{p.adj.signif}",  step.increase = 0.05, tip.length = 0.01, size = 6) +
  xlab("") +
  ...
  scale_fill_manual(values = c("FS - 25 µL" = "#E41A1C",
                               ...
                               "FS - 5 µL" = "#FF850D",
                               "SS3" = "#4DAF4A"))
ggsave(p.isoform.HEK, filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/HEK_isoforms.pdf", dpi = 450, width = 11, height = 8, bg = "white")
...
ggsave(plot_grid(p2 + theme(axis.title.x = element_blank()), saturation, nrow = 2), filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/HEK_saturation_downsampling.pdf", dpi = 500, height = 8, width = 10, bg = "white")
...
p6 <- corrs %>%
  ...
  ggpubr::stat_pvalue_manual(p.signif, label = "{p.adj.signif}",  step.increase = 0.02, tip.length = 0.01, size = 5)
...
p1 <- ggplot() +
  ...
  ggpubr::stat_pvalue_manual(filter(du, threshold == "count > 0"), label = "{p.adj.signif}",  step.increase = 0.075, tip.length = 0.01, size = 4.5) +  
  ggpubr::stat_pvalue_manual(filter(du, threshold == "count > 5"), label = "{p.adj.signif}",  step.increase = 0.08, tip.length = 0.01, size = 4.5) 
...
"FS-19c" = "#B10026")) +
  ylab("Cell-to-Cell Correlation (tau)") +
    ggpubr::stat_pvalue_manual(p.signif, label = "p = {p.adj.signif}",  step.increase = 0.04, tip.length = 0.01, size = 5) 
