...
p.diversity <- res %>%
    ...
    coord_flip() +
    theme_cowplot(font_size = 20) +
    theme(panel.border = element_rect(size = 0.5, color = "black"), axis.title.x = element_text(size = 16), axis.text.x = element_text(size = 16), strip.background = element_blank(), strip.text = element_blank(), legend.position = "none", legend.direction = "horizontal", legend.title = element_blank(), axis.ticks.y = element_blank()) +
...
ggsave(plot_grid(p.diversity + theme(legend.position = "right") + ylab("Gene Diversity")  + theme(legend.position = "none"), 
                 p.detected + theme(legend.position = "right")+ ylab("Genes Detected") + theme(legend.position = "none"), nrow = 2), filename = "/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/ORGANOIDS_detected_diversity.downsample.pdf",
       dpi = 450, width = 16, height = 12, bg = "white")
...
p.gene.per.cell <- 
  ...
  ggpubr::stat_pvalue_manual(p.signif, label = "p = {p.adj.signif}", step.increase = 0.025, step.group.by = "cell_type.internal", tip.length = 0.01, size = 4) +
