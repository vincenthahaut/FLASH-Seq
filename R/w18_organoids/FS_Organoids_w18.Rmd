...
p.dimplot.celltype <- plot_grid(ncol = 3,
   DimPlot(ssce.FS.all, group.by = "cell_type") + scale_color_manual(values = celltype.colors()) + theme(legend.position = "none"),
 DimPlot(ssce.FS.internal, group.by = "cell_type") + scale_color_manual(values = celltype.colors()) + theme(legend.position = "none"),
  DimPlot(ssce.FS.umi, group.by = "cell_type") + scale_color_manual(values = celltype.colors()) + theme(legend.position = "none"))

ggsave(p.dimplot.celltype, filename = "/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/ORGANOIDS_dimplot.celltype.tiff", bg = "white", dpi = 300, width = 18, height = 6)
...
ggsave(DimPlot(ssce.FS.all, group.by = "cell_type", pt.size = 1.25) + scale_color_manual(values = celltype.colors()), dpi = 300, width = 12, height = 8, bg = "white",
...
ggsave(p.celltypes, bg = "white", filename = "/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/ORGANOIDS_cell_types.perc.tiff", dpi = 200, height = 5, width = 10)
```
...
p.logFcRelation <- 
  ...
  geom_text(data = data_frame(), aes(x = 7, y = 1, label = "Pearson:0.7147"), size = 6) +
  geom_text(data = data_frame(), aes(x = 7, y = 2, label = "Spearman:0.6246"), size = 6)

ggsave(p.logFcRelation, filename = "/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/ORGANOIDS_pLogFC.tiff", dpi = 300, height = 6, width = 8, bg= "white")
...
ggsave(p.logFold.distrib, filename = "/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/logFold.distribution.tiff", dpi = 300, height = 4, width = 6, bg = "white")
...
for(i in unique(names(markers.celltype.l))){
  ...
  uu.l[[i]] <- UpSetR::upset(UpSetR::fromList(markers.celltype.l[[i]]), 
              order.by = "freq",
              text.scale = c(3,3,3,1.5,2,2),
              set_size.angles = 90,
              sets.bar.color = as.vector(celltype.colors()[names(celltype.colors()) == i]))
}

# Tiff was not working in the loop
tiff(filename = paste0("/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/UpsteR.photobipolarFate.tiff"), bg = "white")
uu.l[[1]]
dev.off()
tiff(filename = paste0("/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/UpsteR.", names(uu.l)[2], ".tiff"), bg = "white")
uu.l[[2]]
dev.off()
tiff(filename = paste0("/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/UpsteR.", names(uu.l)[3], ".tiff"), bg = "white")
uu.l[[3]]
dev.off()
tiff(filename = paste0("/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/UpsteR.", names(uu.l)[4], ".tiff"), bg = "white")
uu.l[[4]]
dev.off()
tiff(filename = paste0("/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/UpsteR.", names(uu.l)[5], ".tiff"), bg = "white")
uu.l[[5]]
dev.off()
tiff(filename = paste0("/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/UpsteR.", names(uu.l)[6], ".tiff"), bg = "white")
uu.l[[6]]
dev.off()
tiff(filename = paste0("/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/UpsteR.", names(uu.l)[7], ".tiff"), bg = "white")
uu.l[[7]]
dev.off()
tiff(filename = paste0("/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/UpsteR.", names(uu.l)[8], ".tiff"), bg = "white")
uu.l[[8]]
dev.off()
tiff(filename = paste0("/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/UpsteR.horizamacrineFate.tiff"), bg = "white")
uu.l[[9]]
dev.off()
tiff(filename = paste0("/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/UpsteR.", names(uu.l)[10], ".tiff"), bg = "white")
uu.l[[10]]
dev.off()
tiff(filename = paste0("/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/UpsteR.", names(uu.l)[11], ".tiff"), bg = "white")
uu.l[[11]]
dev.off()
...
p.overlap <- overlap.properties %>%
  ...
  ungroup() %>%
  ggplot(aes(x = gene_type, y = perc, fill = TYPE)) +
...
ggsave(filename = paste0("/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/Gene_features.tiff"),
  p.overlap,
  dpi = 200, width = 6, height = 4, bg = "white")
...
p.diversity <- res %>%
  ...
  scale_fill_manual(values = c("SMART" = "#FF850D", 
                               "DROPLET" = "#2ac7c2")) +
  theme_cowplot(font_size = 20) +
...
ggsave(p.diversity, filename = "/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/ORGANOIDS_diversity.tiff",
       dpi = 200, width = 8, height = 6, bg = "white")
...
p.signif <- res %>%
  ...
  mutate(p.adj = p.adjust(p, method = "bonferroni"),
         p.adj.signif = ifelse(p.adj.signif == "****", "***", p.adj.signif)) %>%
  group_by(cell_type) %>%
  mutate(y = seq(7000,7000+250*(n()-1),250)) %>%
  ungroup() %>%
  mutate(xstart = 0.675,
         xend = xstart+(match(group2, levels(factor(res$name)))-1)/3)
...
p.diversity <- res %>%
  ...
  ylab("Gene Diversity") + ylab("") +
  theme(legend.position = "none", 
        legend.title = element_blank(), 
        axis.ticks.x = element_blank(),
        axis.text.x = element_blank()) +
  geom_segment(data = p.signif, aes(x = xstart, xend = xend, y=y,yend=y)) + 
  geom_text(data = p.signif, aes(x = (xstart+xend)/2, y=y*1.005, label = p.adj.signif), size = 7) 

ggsave(p.diversity, filename = "/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/ORGANOIDS_diversity.downsample.tiff",
       dpi = 200, width = 20, height = 6, bg = "white")
...
med <- med %>%
  ...
  mutate(name = case_when(
    name == "count_0.all" ~ "FS-UMI\nAll\nGenes",
    name == "count_0.internal" ~ "FS-UMI\nInternal\nGenes",
    name == "count_0.umi" ~ "FS-UMI\nUMI\nGenes",
    name == "count_0.salmon"~ "FS-UMI\nAll\nTranscripts",
    name == "10x Genomics" ~ "10x\nGenomics\nGenes"),
    name = factor(name, levels = c("10x\nGenomics\nGenes", "FS-UMI\nUMI\nGenes", "FS-UMI\nInternal\nGenes", "FS-UMI\nAll\nGenes", "FS-UMI\nAll\nTranscripts"))) 

# 6. Statistical significance

# 6.1. Wilcoxon-test paired intra FS
p.signif <- ggpubr::compare_means(data = med, value ~ name  , group.by = "cell_type.internal") %>%
  filter(group1 != "FS-UMI\nAll\nTranscripts", group2 != "FS-UMI\nAll\nTranscripts") %>%
  # Readjust pvals as one group was dropped
  mutate(p.adj = p.adjust(p, method = "bonferroni"),
  ...
  mutate(y = seq(15000,15000+800*(n()-1),800)) %>%
  ungroup() %>%
  mutate(xstart = match(group1, levels(med$name)),
         xend = match(group2, levels(med$name))) 

# 7. Visualize results
p.gene.per.cell <- 
  ggplot() +
  geom_hline(yintercept = seq(0,20000,2000), linetype = "dashed", color = "darkgrey") + 
  geom_violin(data =med, aes(x = name, y = value, fill = name)) +
  facet_wrap("cell_type.internal", nrow = 2) +
  geom_boxplot(data =med, aes(x = name, y = value, fill = name), width = 0.05) +
  ...
  geom_segment(data = p.signif, aes(x = xstart, xend = xend, y=y,yend=y)) + 
  geom_text(data = p.signif, aes(x = (xstart+xend)/2, y=y*1.0025, label = p.signif), size = 7) 

# 8. Save
ggsave(p.gene.per.cell, dpi = 200, height = 12, width = 24, bg = "white",
       filename = "/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/ORGANOIDS_totalGenes.tiff")

  
ggsave(p.proportion, dpi = 200, height = 6, width = 6, bg = "white",
...
ggsave(DimPlot(ssce, group.by = "cell_type") + scale_color_manual(values = celltype.colors()), bg = "white",
  dpi = 200, width = 10, height = 6, filename = "/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/DimPlot.transcriptome.tiff")
...
p.exon <- plot_grid(rel_widths = c(0.66,0.33),
...
ggsave(p.exon, filename = "/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/ORGANOIDS_PKM_exons.tiff", dpi = 200, height = 6, width = 16, bg = "white")
...
ggsave(z, filename = "/home/vincent.hahaut/data_storage/ORGANOIDS/FIGURES/correlation_umi_internal.tiff", bg = "white", dpi = 300, height = 7, width = 14)
