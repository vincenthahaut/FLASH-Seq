#' Create GTF Exon-Intron file
#' 
#' Create a protein-coding GTF containing the reduce exon-intron 
#' Adapted from Devon Ryan (https://www.biostars.org/p/165226/) - parallelised and allow genes with no introns.
#' @param gtf.path, GTF file from gencode containing only the protein_coding genes
#' @param prefix, output path prefix
#' @param threads, number of threads to use for the intron search

createGTFfile <- function(gtf.path = "/home/vincent.hahaut/data_storage/REFERENCES/hsap_fastsmart/REFERENCE/GTF/gencode.v34.primary_assembly.annotation.protein_coding.gtf", prefix = NULL, threads = 30){
  ...
  # 7. Save as GTF
  print("7. Save data")
  write.table(intron.gtf, paste0(prefix, "gencode.v34.introns.gtf"), sep = "\t", col.names = F, row.names = F, quote = F)
  write.table(exon.gtf, paste0(prefix, "/gencode.v34.exons.gtf"), sep = "\t", col.names = F, row.names = F, quote = F)
  write.table(unified, paste0(prefix, "gencode.v34.exonsANDintrons.gtf"), sep = "\t", col.names = T, row.names = F, quote = F)
