...
invasion.plot <- function(group, suffix = "group1"){
  ...
  ggsave(p.perfect, filename = paste0( "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/StrandInvasion_Perfect.", suffix,".tiff"), dpi= 300, height = 5.4*length(unique(filter(dat,GROUP %in% group)$SUBGROUPS)), width = 12)
  ggsave(p.all, filename = paste0("/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/StrandInvasion_Mismatch.", suffix,".tiff"), dpi= 300, width = 14, height = 5.5)
  ggsave(p.all.GGG, filename = paste0("/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/StrandInvasion_Mismatch.", suffix,".GGG.tiff"), dpi= 300, width = 14, height = 5.5)
...
ggsave(plot_grid(p[[1]], p[[2]],p[[3]], p[[4]], nrow = 1), bg = "white", filename = paste0("/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/seqlogo_upstream.GGG.tiff"), dpi= 300, height = 4, width = 20)
...
ggsave(p.logo, bg = "white", filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/seqlogo_upstream.tiff", dpi= 300, height = 24, width = 36)
```
...
intron.exon.dat.tmp <- filter(intron.exon.dat, GROUP %in% group1)
p.signif <- intron.exon.dat.tmp %>%
  ...
  mutate(p.signif = case_when(p.adj < 0.05 & p.adj > 0.005 ~ "*",
                      ...
                      p.adj < 0.0005 ~ "***",
                      TRUE ~ "ns"),
         x = match(group1, levels(factor(intron.exon.dat.tmp$GROUP))),
         xend = match(group2, levels(factor(intron.exon.dat.tmp$GROUP))),
         y = case_when(
           SUBGROUP == "EXON - Concordant" ~ seq(97, 97+(14), 1),
           SUBGROUP == "EXON - Discordant" ~ seq(5, 5+(0.5*14), 0.5),
           SUBGROUP == "INTRON - Concordant" ~ seq(13, 13+(0.5*14), 0.5),
           SUBGROUP == "INTRON - Discordant" ~ seq(26, 26+(0.75*14), 0.75)
         )) %>%
  ungroup() %>%
  filter(p.signif != "ns")
...
# 4. Visualize:
p.intron <- ggplot() + 
  geom_violin(data = filter(intron.exon.dat, GROUP %in% group1), aes(x = GROUP, y = perc, fill = SUBGROUP)) + 
  geom_boxplot(data= filter(intron.exon.dat, GROUP %in% group1), aes(x = GROUP, y = perc, fill = SUBGROUP), width = 0.1, position = position_dodge(0.9)) +
  ...
  ylab("Percentage 5'UMI-Reads") +
  xlab("") +
  guides(fill =guide_legend(nrow = 2)) +
  geom_segment(data = p.signif, aes(y = y, x = group1, xend = group2, yend = y))  +
  geom_text(data = p.signif, aes(y = y, x = (x+xend)/2, label = p.signif)) 


ggsave(p.intron, bg = "white", filename = paste0( "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/strandInvasion_intronicReads.group1.tiff"), dpi= 300, height = 11, width = 13)
...
ggsave(plogo, filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/ggseqlogo.tiff", dpi = 300, height = 16, width = 24)
...
ggsave(p.motifs, filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/S1_Motif_Position.tiff", dpi = 300, height = 16, width = 24)
```
