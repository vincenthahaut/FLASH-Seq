...
invasion.plot <- function(group, suffix = "group1"){
  ...
  ggsave(p.perfect, filename = paste0( "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/StrandInvasion_Perfect.", suffix,".tiff"), dpi= 450, height = 5.4*length(unique(filter(dat,GROUP %in% group)$SUBGROUPS)), width = 12)
  ggsave(p.all, filename = paste0("/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/StrandInvasion_Mismatch.", suffix,".tiff"), dpi= 450, width = 14, height = 5.5)
  ggsave(p.all.GGG, filename = paste0("/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/StrandInvasion_Mismatch.", suffix,".GGG.tiff"), dpi= 450, width = 14, height = 5.5)
...
ggsave(plot_grid(p[[1]], p[[2]],p[[3]], p[[4]], nrow = 1), bg = "white", filename = paste0("/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/seqlogo_upstream.GGG.tiff"), dpi= 450, height = 4, width = 20)
...
ggsave(p.logo, bg = "white", filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/seqlogo_upstream.tiff", dpi= 450, height = 24, width = 36)
```
...
1391221 <- read_rds("/home/vincent.hahaut/data_storage/FS_intermediate_file/bias.umi.exon_intron_unstranded.rds")
...
for(SUB in c("EXON - Concordant" ,  "EXON - Discordant",   "INTRON - Concordant", "INTRON - Discordant")){
  ...
  ylab("5'UMI-Reads (%)") +
  xlab("") +
  ggpubr::stat_pvalue_manual(filter(p.signif, SUBGROUP == SUB), label = "{p.adj.signif}",  step.increase = ifelse(SUB == "EXON - Concordant", 0.1, 0.032), tip.length = 0.01, size = 4) 
}


ggsave(plot_grid(nrow = 1,  p.sub[[1]], p.sub[[2]], p.sub[[3]], p.sub[[4]]), bg = "white", filename = paste0( "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/strandInvasion_intronicReads.group1.tiff"), dpi= 450, height = 6, width = 18)
...
ggsave(plogo, filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/ggseqlogo.tiff", dpi = 450, height = 16, width = 24)
