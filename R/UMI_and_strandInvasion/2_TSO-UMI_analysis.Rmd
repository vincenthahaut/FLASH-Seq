...
relationShipInternalUMI(group4, suffix = "group4", width = 18, h = 20, facets.row = 4)
...
```{r poster Genes - downsamplings , echo = FALSE, include = TRUE, warning = FALSE, comment = FALSE, message = FALSE, cache = FALSE}

# 1. Read number of detected genes per sample
df.umi <- read_tsv("/home/vincent.hahaut/data_storage/FS_intermediate_file/UMI.txt") %>% dplyr::select(-MOLECULES)
df.internal <- read_tsv("/home/vincent.hahaut/data_storage/FS_intermediate_file/ALL.txt") %>%
  filter(TYPE == "INTERNAL")
colnames(df.umi) <- colnames(df.internal)

# 2. Aggregate counts
df.counts <- bind_rows(df.umi, df.internal) %>%
  mutate(GROUP = case_when(
    grepl("5_SS3|6_SS3", ID) ~ "SS3",
    grepl("SS3fwd", ID) ~ "SS3\ndata from\nHagemann-J. et al",
    grepl("STRToligoT_Spacer2", ID) ~ "TSO-ATAAC\nSTRT-dT",
...
p.signif <- df.counts %>%
  group_by(TYPE) %>%
  rstatix::wilcox_test(count.0.exon ~ GROUP) %>%
  ungroup() %>%
...
p.genes.poster <- ggplot() +
  geom_violin(data = df.counts, aes(x = GROUP, y = count.0.exon, fill = GROUP)) +
  geom_boxplot(data = df.counts, aes(x = GROUP, y = count.0.exon, fill = GROUP), width = 0.1) +
  facet_wrap("TYPE") + 
  ...
  ylab("Genes Detected") +
  scale_fill_manual(values = c("TSO-CTAAC\nSTRT-dT" = "#E41A1C",
                               "SS3\ndata from\nHagemann-J. et al" = "#4DAF4A")) +
    ggpubr::stat_pvalue_manual(p.signif, label = "{p.adj.signif}", step.group.by = "TYPE", tip.length = 0.01, size = 5) 


# 7. Save results
ggsave(p.genes.poster, bg = "white",
  filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/HEK_downsampling_UMI_internal.tiff",
  dpi = 450, height = 6, width = 10)
```
...
p.molecules <- ggplot() +
  ...
  geom_text(data = p.signif, aes(x = 75000, y = 230000, label = p.adj.signif), size = 6.5)
...
p.numberGenes <- ggplot() + 
  ...
  ggpubr::stat_pvalue_manual(p.signif, label = "{p.adj.signif}",  xmin = "xmin", xmax = "xmax", tip.length = 0.01, size = 5.5) +
...
p.genetypes <- ggplot() + 
  ...
  geom_text(data = filter(p.signif, name == "Protein Coding Gene"), aes(x = (x+xend)/2, y = y, label = p.adj.signif), size = 5) +
  geom_segment(data = filter(p.signif, name != "Protein Coding Gene"), aes(x = x, xend = xend, y = y+0.09*y, yend = y+0.09*y)) +
  geom_text(data = filter(p.signif, name != "Protein Coding Gene"), aes(x = (x+xend)/2, y = y+0.13*y, label = p.adj.signif), size = 5)
...
ggsave(p.dedup, filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/HEK_UMI_editDistanceUMI.pdf", bg = "white", dpi = 450, height = 6, width = 4)
...
results.aggr <- results %>%
  mutate(GROUP = case_when(
    ...
    grepl("276_LA", ID) ~ "LA-TSO-CAGCA\nSTRT-dT",
    grepl("SS3oligoT_Fstso", ID) ~ "TSO-FS-UMI\nSS3-dT"))  %>%
  filter(GROUP %in% c("SS3 data from\nHagemann-J. et al","TSO-CATCA\nSTRT-dT","TSO-CTAAC\nSTRT-dT","TSO-CAGCA\nSTRT-dT")) %>%
...
p.isoforms <- ggplot() + 
  ...
  xlab("") +
  scale_y_continuous(labels = scales::comma) +
  theme(legend.position = "none", strip.text = element_text(size = 14), axis.text.x = element_text(angle = 45, hjust = 1, size = 11)) + 
...
ggsave(p.isoforms, filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/HEK_UMI_isoforms.pdf", dpi = 450, height = 7, width = 8, bg = "white")
```
