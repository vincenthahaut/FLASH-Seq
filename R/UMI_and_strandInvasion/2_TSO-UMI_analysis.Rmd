...
p.numberGenes <- ggplot() + 
  ...
  scale_y_continuous(breaks = seq(0,5000,1000), limits = c(0,7500)) +
...
p.numberLength <- ggplot() + 
  ...
  geom_boxplot(data = overlap.aggr.tmp, aes(x = GROUP, y = value, fill = name)) + 
  scale_y_continuous(breaks = seq(5000,7000,1000), limits = c(4500,8000)) +
  theme_bw(base_size = 20) +
  scale_fill_brewer(palette = "Set1") +
  facet_wrap("name") +
...
dedup.dist.aggr <- bind_rows(dedup.dist) %>%
  ...
  mutate(GROUP = ifelse(grepl("SS3", GROUP), "SS3 (n = 209)", "FS-UMI (n = 79)")) %>%
  group_by(ID) %>%
