...
relationShipInternalUMI <- function(condition = group1, suffix = "group1", facets.row = 5, width = 12, h = 12){
  ...
  # 5. Save it
  ggsave(p.internal.umi, dpi = 450, filename = paste0("/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/UMI.internalvsUMI.", suffix, ".tiff"), bg = "white", width = width, height = h)
...
relationShipInternalUMI(group5, suffix = "group5", width = 12, h = 12, facets.row = 2)
...
p.molecules <- ggplot() +
  ...
  geom_text(data = p.signif, aes(x = 75000, y = 230000, label = p.adj.signif), size = 4)


ggsave(
  p.molecules, bg = "white",
  filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/HEK_uniquemolecules.tiff",
  dpi = 450, height = 7, width = 18)
...
overlap.aggr.tmp <- overlap.aggr %>%
  dplyr::select(GROUP, Internal.nGenes, UMI.nGenes, overlap.nGenes, ALL.nGenes) %>%
  pivot_longer(-GROUP) %>%
...
p.numberGenes <- ggplot() + 
  ...
  scale_y_continuous(breaks = seq(0,5000,1000), limits = c(0,7500), labels = scales::comma) +
  ...
  ggpubr::stat_pvalue_manual(filter(p.signif, name == "UMI Specific Genes"), label = "{p.adj.signif}",  step.increase = 0.075, tip.length = 0.01, size = 4) +
  ggpubr::stat_pvalue_manual(filter(p.signif, name == "Internal Specific Genes"), label = "{p.adj.signif}",  step.increase = 0.075, tip.length = 0.01, size = 4) +
  ggpubr::stat_pvalue_manual(filter(p.signif, name == "Overlaping Genes"), label = "{p.adj.signif}",  step.increase = 0.075, tip.length = 0.01, size = 4) +
  ggpubr::stat_pvalue_manual(filter(p.signif, name == "UMI+Internal Genes"), label = "{p.adj.signif}",  step.increase = 0.075, tip.length = 0.01, size = 4) +
  ggtitle("Genes Detected (> 0 read)")

# 2. Length Gene
overlap.aggr.tmp <- overlap.aggr %>%
  dplyr::select(GROUP, Internal.length, UMI.length, overlap.length, all.length) %>%
  pivot_longer(-GROUP) %>%
  filter(value < 25000) %>%
...
p.numberLength <- ggplot() + 
  ...
  geom_boxplot(data = overlap.aggr.tmp, aes(x = GROUP, y = value, fill = name)) + 
  scale_y_continuous(breaks = seq(5000,7000,1000), limits = c(4500,8000), labels = scales::comma) +
  theme_bw(base_size = 20) +
  scale_fill_brewer(palette = "Set1") +
  facet_wrap("name") +
  xlab("") + ylab("Gene Length (bp)") + 
  theme(legend.title = element_blank(), legend.position = "none", axis.text.x = element_text(angle = 45, hjust = 1, size = 12)) +
  ggpubr::stat_pvalue_manual(filter(p.signif, name == "Internal Specific Genes"), label = "{p.adj.signif}",  step.increase = 0.06, tip.length = 0.01, size = 4) +
  ggpubr::stat_pvalue_manual(filter(p.signif, name == "Internal Specific Genes"), label = "{p.adj.signif}",  step.increase = 0.06, tip.length = 0.01, size = 4) +
  ggpubr::stat_pvalue_manual(filter(p.signif, name == "Overlaping Genes"), label = "{p.adj.signif}",  step.increase = 0.06, tip.length = 0.01, size = 4) +
  ggpubr::stat_pvalue_manual(filter(p.signif, name == "UMI+Internal Genes"), label = "{p.adj.signif}",  step.increase = 0.06, tip.length = 0.01, size = 4) +
  ggtitle("Gene Lengths")


# 3. GC Content
overlap.aggr.tmp <- overlap.aggr %>%
  dplyr::select(GROUP, Internal.GC, UMI.GC, overlap.GC, all.GC) %>%
  pivot_longer(-GROUP) %>%
...
p.genetypes <- ggplot() + 
  ...
  geom_segment(data = filter(p.signif, name != "Protein Coding Gene"), aes(x = x, xend = xend, y = y-0.1*y, yend = y-0.1*y,)) +
  geom_text(data = filter(p.signif, name != "Protein Coding Gene"), aes(x = (x+xend)/2, y = y, label = p.adj.signif), size = 4)


ggsave(plot_grid(plot_grid(p.numberGenes, p.numberLength, p.GCperc, nrow = 1), ncol = 1, p.genetypes, rel_heights = c(0.6,0.4)),
...
ggsave(p.dedup, filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/HEK_UMI_editDistanceUMI.tiff", bg = "white", dpi = 450, height = 8, width = 6)
...
ggsave(p.isoforms, filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/HEK_UMI_isoforms.tiff", dpi = 450, height = 9, width = 10, bg = "white")
```
...
percentageUMIReads <- function(condition = group1, suffix = "group1"){
  ...
  # 10. Save it
  ggsave(p.readUMI, filename = paste0( "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/UMI_READS.", suffix, ".tiff"), dpi = 450, width = 1.1*length(condition), height = 7, bg = "white")
...
plotCoverageUMI_INTERNAL <- function(condition = group1, suffix = "group1"){
  ...
  # 5. Save it
  ggsave(p.coverage, filename = paste0("/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/UMI_COVERAGE.", suffix, ".tiff"), width = 16, height = 9, bg = "white")
...
plotReadDistribUMI_INTERNAL <- function(condition = group1, suffix = "group1"){
  ...
  ggsave(plot_grid(nrow = 3,
    ...
    p.readDistrib.l[[4]], 
    p.readDistrib.l[[5]]), 
    filename = paste0("/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/UMI_readDistrib.", suffix, ".tiff"), width = 18, height = 28, bg = "white", dpi = 450)
...
plotSTARfsUMIinfo <- function(condition = group1, suffix = "group1"){
  ...
  ggsave(plot_grid(
    ...
    p.star.l[[2]], 
    p.star.l[[3]], nrow = 3), 
    filename = paste0("/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/UMI_STAR.", suffix, ".tiff"), width = 18, height = 24, bg = "white", dpi = 450)
}
...
# 3. Save
ggsave(pcDNA, filename = "/home/vincent.hahaut/data_storage/FS_intermediate_file/FIGURES/HEK_cDNA_yield.TSOumi.tiff", width = 20, height = 8, dpi = 450)
```
